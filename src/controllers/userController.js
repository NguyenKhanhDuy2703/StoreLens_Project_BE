...
const getStoreForUser = async (req , res) => {
  try{
    ...
    if (role.toLowerCase() !== "admin"){
     listStore = await storeModel.find({ "manager_infor.email": email }).select({ _id : 0  , __v : 0  }).lean();
    }else{
      listStore = await storeModel.find().select({ _id : 0  , __v : 0  }).lean();
    }
    return res.status(200).json({ message: "Get store for user successfully" , data : {role , listStore} });
  }catch(error){
